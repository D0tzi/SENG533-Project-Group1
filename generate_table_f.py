...
def parse_ycsb_log(log_file_path):
    ...
    metrics_patterns = {
        ...
        'Read-Modify-Write Ops': re.compile(r'\[READ-MODIFY-WRITE\], Operations, (\d+)'),
        'Read-Modify-Write Latency': re.compile(r'\[READ-MODIFY-WRITE\], AverageLatency\(us\), (\d+\.\d+)'),
...
log_file_path = "data_f.txt"
df = parse_ycsb_log(log_file_path)

df.to_csv('ycsb_performance_results_workload_f.csv', index=False)
